      ...
      <div className="navwrapper flex flex-col items-center gap-20 ">
        ...
        <ul className='flex flex-col items-start pl-4 space-y-2 2xl:space-y-5 relative '>
          ...
          {links.map((item, index) => (
            ...
            <li onClick={() => FocusItem(index)} key={item.id} className={focusItem === index ? 'bg-primary pl-6 py-3 lg:py-3 2xl:py-4  hover:bg-primary  ease-in-out duration-300 rounded-tl-md rounded-bl-md w-[100%] lg:pr-8 pr-2' : 'linc  pl-6 py-3 lg:py-3 2xl:py-4 hover:bg-primary  ease-in-out duration-150  rounded-tl-md rounded-bl-md w-[100%] '}><a href={item.anchor} className='flex gap-7 cursor-pointer focus:bg-primary '><img className='h-5 lg:h-4 2xl:h-5 inline ' src={item.icon} alt="" /> <p className='text-sm uppercase 2xl:text-md w-[100%] group-hover:flex item-start pr-8 flex md:hidden  '>{item.title}</p></a></li>
